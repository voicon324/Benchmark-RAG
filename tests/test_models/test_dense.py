...
class TestDenseTextRetriever:
    ...
    def trust_remote_code_config(self):
        """Configuration with trust_remote_code enabled."""
        return {
            "name": "test_dense_trust",
            "type": "dense",
            "model_name_or_path": "custom-model/with-remote-code",
            "device": "cpu",
            "batch_size": 4,
            "parameters": {
                "model_type": "sentence_transformer",
    ...
    def test_init_trust_remote_code_config(self, trust_remote_code_config):
        """Test initialization with trust_remote_code enabled."""
        model = DenseTextRetriever(trust_remote_code_config)
        
        assert model.name == "test_dense_trust"
        assert model.model_name_or_path == "custom-model/with-remote-code"
        assert model.trust_remote_code is True
        assert model.is_loaded is False
    
    def test_init_trust_remote_code_default(self, basic_config):
    ...
    def test_load_model_with_trust_remote_code(self, mock_sbert, trust_remote_code_config):
        """Test loading sentence transformer model with trust_remote_code=True."""
        # Setup mock
        mock_model = Mock()
